...
# Single-file GUI launcher for AESTHETIC (root-level app.py)
# - Always loads E:\AestheticApp\config.yaml (no env var lookups)
# - Background thread for pipeline
# - Logs to outputs/logs/run.log
# - Copy Error / Open Output buttons
...
# ---- project locations ----
APP_DIR = Path(__file__).resolve().parent          # E:\AestheticApp
PROJECT_ROOT = APP_DIR                             # same as app dir
DEFAULT_CONFIG_PATH = PROJECT_ROOT / "config.yaml" # <<< ALWAYS this file

# ---- import pipeline from package code ----
...
def load_config(config_path: Optional[Path] = None) -> Dict[str, Any]:
    """
    Always load config from the project's own config.yaml unless an explicit
    path is provided by the caller. Environment variables are ignored.
    """
    ...
    # normalize output folder to absolute now, so all modules agree
...
class App(tk.Tk):
    def __init__(self) -> None:
        ...
        ttk.Entry(root, textvariable=self.source_var, width=70).grid(row=1, column=0, columnspan=3, sticky="ew")
    ...
    def _logblock(self, text: str) -> None:
        try:
            self.log.insert("end", "\n" + text.rstrip() + "\n")
            self.log.see("end")
        except Exception:
