...
class MainWindow(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Entry(input_frame, textvariable=self.source_var, width=64).grid(row=0, column=0, sticky="ew", padx=6, pady=6)
        ...
        extra_buttons.grid(row=1, column=1, sticky="e", pady=8)
        ...
        ttk.Button(extra_buttons, text="Kill CLIP Worker", command=self._kill_clip_worker).pack(side="left", padx=6)
        ttk.Button(extra_buttons, text="Show Manifest", command=self._show_manifest).pack(side="left", padx=6)

        ttk.Label(container, textvariable=self.status_var).grid(row=2, column=0, sticky="w")
        ttk.Progressbar(container, variable=self.progress, maximum=1.0).grid(row=2, column=0, sticky="we", pady=(4, 8))
        ...
        status_frame.grid(row=0, column=1, rowspan=4, sticky="nsew")
    ...
    def _kill_clip_worker(self) -> None:
        self._log("CLIP worker uses per-run subprocesses; nothing to kill.")

    def _show_manifest(self) -> None:
        if not self.last_manifest:
            messagebox.showinfo("Manifest", "Run the pipeline to create a manifest.")
            return
        messagebox.showinfo("Manifest", self.last_manifest)

    def _rerun_with_seed(self) -> None:
        seed = simpledialog.askinteger("Seed", "Enter seed", initialvalue=int((self.config_dict.get("sampling") or {}).get("random_seed", 42)))
        if seed is not None:
    ...
    def _run_async(self) -> None:
        ...
        if not source:
            messagebox.showerror("AESTHETIC", "Please select a source file or URL")
            return
    ...
    def _on_success(self, result: Dict[str, Any]) -> None:
        ...
        for path_info in result.get("written", []):
            self._log(f"Saved: {path_info.get('path')} score={path_info.get('score'):.3f}")
