...
Why a worker?
- You reported hard crashes (no Python traceback) near CLIP. That’s typical of
  native/CUDA faults. By moving CLIP into a *subprocess*, the GUI stays alive,
  we capture stderr to file, and we can continue/fail gracefully.
...
- On failure, it logs a short message via on_progress and returns without raising.

Worker:
- aesthetic/features/clip_worker.py (run via `python -m ...`), writes feats.npy
  and logs all stdout/stderr to a per-run log file under outputs/logs/.
"""
...
def embed_frames_if_enabled(items: List[Dict[str, Any]], on_progress: ProgressCb = None) -> None:
    ...
    batch_sz = int(gpu.get("batch_size", 8))
    ...
    # Prepare IO & logs
    out_dir  = (cfg.get("output") or {}).get("folder", "aesthetic/outputs")
    ...
    err_log  = os.path.join(tmp_dir, "stderr.log")
    ...
    try:
        ...
        _log(on_progress, 0.02, f"CLIP worker launch failed; see {err_log}")
        return

    if rc != 0 or not os.path.exists(out_npy):
        _log(on_progress, 0.02, f"CLIP worker failed (rc={rc}); see {err_log}")
        return
    ...
    try:
        ...
        if feats.shape[0] != len(idxs):
            _log(on_progress, 0.02, f"CLIP mismatch N={feats.shape[0]}!={len(idxs)}; see {err_log}")
            return
        for j, i in enumerate(idxs):
            items[i]["feat_clip"] = feats[j]
        _log(on_progress, 0.2, f"CLIP worker done → feats={feats.shape}")
    except Exception:
        _log(on_progress, 0.02, f"CLIP readback error; see {err_log}")
        return
# end 1–240
